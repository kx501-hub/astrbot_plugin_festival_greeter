...
def validate_document(body):
    ...
    if all(isinstance(item, str) for item in holidays):
        ...
        for token, name in zip(holidays[::2], holidays[1::2]):
            if len(token) != 4 or not token.isascii() or not token.isdigit() or not name.strip():
        ...
        for item in holidays:
            ...
            if not isinstance(item.get("aliases", []), list) or any(not isinstance(x, str) for x in item.get("aliases", [])):
    ...
    for item in birthdays:
        ...
        if len(parts) != 3 or not parts[0] or parts[1] != "GroupMessage" or not parts[2]:
...
class FestivalPageAPI:
    def __init__(self, plugin):
        ...
        for route, handler, method in (
            ...
            plugin.context.register_web_api(
                f"/astrbot_plugin_festival_greeter/{route}", handler, [method],
    ...
    def document(self):
        ...
        return deepcopy({key: config.get(key, []) for key in ("custom_holidays", "birthdays")})
    ...
    async def settings(self):
        ...
        return json_response({
            **document, "revision": revision(document),
            ...
            "targets": self.plugin._apply_group_filter(self.plugin._delivery_targets),
        })
    ...
    async def save(self):
        try:
            ...
            if body.get("revision") != revision(self.document()):
                return error_response("配置已在其他页面修改，请重新载入后再保存", status_code=409)
            calendar = HolidayCalendar.from_config(document["custom_holidays"], document["birthdays"])
    ...
    async def preview(self):
        try:
            ...
            start = date.fromisoformat(body.get("start", self.plugin._now().date().isoformat()))
            ...
            for offset in range(days):
                for occurrence in calendar.get_holidays_for(start + timedelta(days=offset)):
                    if self.plugin._holiday_repeat_mode == "first-day" and not occurrence.is_first_day:
                        continue
                    definition = occurrence.definition
                    targets = self.plugin._apply_group_filter(
                        [definition.target_session] if definition.target_session else self.plugin._delivery_targets
                    )
                    rows.append({
                        **occurrence.to_payload(), "type": definition.greeting_type,
                        "recipient": definition.recipient, "targets": targets,
